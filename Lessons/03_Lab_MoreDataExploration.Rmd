...
```{r}
lowtemp_F<-c(20, 25, 28, 19, 23, 25, 29, 43, 49, 36)
hightemp_F<-c(42, 43, 46, 36, 42, 49, 52, 56, 67, 60)
```
...
Lowtemp_C<-FtoC(lowtemp_F)
Hightemp_C<-FtoC(hightemp_F)
```

Combine your four vectors into a data frame and add informative column names.

```{r}
tempsdf<-data.frame("Low F" = lowtemp_F, "High F" = hightemp_F, "Low C" = Lowtemp_C, "High C" = Hightemp_C)
```
